...
class Trainer:
    def __init__(
        ...
        # lcg loss
        self.lambda_lcg_loss = lambda_lcg_loss
    ...
    def _setup_wandb(self, project, run_name, config):
        ...
        wandb.init(
            project=project, name=run_name, config=config, dir=self.checkpoint_dir
        )
    ...
    def _compute_loss(
        ...
        # lcg loss
        lcg_loss = 0.0
        if use_image and first_layer_output is not None and image_features is not None:
            padding_mask = None
            if attention_mask is not None:
                padding_mask = ~attention_mask.bool()
            lcg_loss = self.model.lcg_loss(
                first_layer_output, image_features, padding_mask
            )
        loss = loss_ntp + self.lambda_lcg_loss * lcg_loss
        return loss, loss_ntp, lcg_loss
    ...
    def train_step(self, batch, use_image):
        ...
        with autocast():
            if use_image:
                outputs, first_layer_output, image_features = self.model(
                    input_ids=input_ids,
            ...
            loss, loss_ntp, lcg_loss = self._compute_loss(
                outputs,
                input_ids,
                attention_mask,
                first_layer_output,
        ...
        return loss.item(), loss_ntp.item(), lcg_loss.item() if use_image else None
    ...
    def train(self):
        ...
        if not self.unified:
            for epoch in range(self.start_epoch, self.max_epochs):
                ...
                epoch_lcg_loss = 0.0
                batch_count = 0
                lcg_loss_batch_count = 0
                progress_bar = tqdm(loader, desc=f"Epoch {epoch + 1}/{self.max_epochs}")
                for batch in progress_bar:
                    loss, loss_ntp, lcg_loss = self.train_step(
                        batch, use_image=train_use_image
                    )
                    epoch_loss += loss
                    epoch_loss_ntp += loss_ntp
                    if train_use_image and lcg_loss is not None:
                        epoch_lcg_loss += lcg_loss
                        lcg_loss_batch_count += 1
                    batch_count += 1
                    self.global_step += 1
                    ...
                    if train_use_image and lcg_loss is not None:
                        log_dict["train/lcg_loss"] = lcg_loss
                    wandb.log(log_dict, step=self.global_step)
                    progress_bar.set_postfix(
                        total_loss=loss,
                        next_token_loss=loss_ntp,
                        lcg_loss=lcg_loss if train_use_image else "N/A",
                ...
                epoch_lcg_loss = (
                    epoch_lcg_loss / max(lcg_loss_batch_count, 1)
                    if train_use_image
                    else None
                )
                ...
                if train_use_image and epoch_lcg_loss is not None:
                    log_dict["train/epoch_lcg_loss"] = epoch_lcg_loss
                wandb.log(log_dict, step=self.global_step)
    ...
    def evaluate_loader(self, loader, use_image, prefix):
        ...
        avg_lcg_loss = (
            total_lcg_loss / max(lcg_loss_batch_count, 1) if use_image else None
        )
        ...
        if use_image and avg_lcg_loss is not None:
            metrics[f"{prefix}/lcg_loss"] = avg_lcg_loss
